...
// 被收入真经篇目的；经典的；按照基督教教会法规的 0号区块
func ReadCanonicalHash(db ethdb.Reader, number uint64) common.Hash {
	var data []byte
	db.ReadAncients(func(reader ethdb.AncientReaderOp) error { // db.AncientStore.Freezer 两级内嵌
		data, _ = reader.Ancient(chainFreezerHashTable, number)
		if len(data) == 0 {
			// Get it by hash from leveldb
			data, _ = db.Get(headerHashKey(number)) // h0n 0号区块在levelDB里的key
		}
		return nil
	})
