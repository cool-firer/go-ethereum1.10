...
func (i *indexEntry) unmarshalBinary(b []byte) {
	i.filenum = uint32(binary.BigEndian.Uint16(b[:2])) // 前面2字节
	i.offset = binary.BigEndian.Uint32(b[2:6]) // 后面4字节
}
...
func (t *freezerTable) repair() error {
	...
	buffer := make([]byte, indexEntrySize) // 6
	...
	if stat.Size() == 0 { // 写进6个0字节
		if _, err := t.index.Write(buffer); err != nil {
			return err
		}
	}
	// Ensure the index is a multiple of indexEntrySize bytes 前面写了6个0字节, 但这里还是0
	...
	t.index.ReadAt(buffer, 0) // read len(buffer) from 0 从0读6个字节
	firstIndex.unmarshalBinary(buffer) // filenum:前2个字节 offset:后4个字节
