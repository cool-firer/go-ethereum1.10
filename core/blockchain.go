...
func NewBlockChain(
	...
	chainConfig *params.ChainConfig, 	// chainConfig
	engine consensus.Engine,					//
	vmConfig vm.Config,	// vm.Config{}
	shouldPreserve func(header *types.Header) bool, // nil
	txLookupLimit *uint64,	// nil
) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	bc := &BlockChain{
		// genesis init时写入的ChainConfig
		chainConfig: chainConfig, // genesis写入的config
	...
	if bc.cacheConfig.SnapshotLimit > 0 {
		...
		bc.snaps, _ = snapshot.New(
			bc.db, 
			bc.stateCache.TrieDB(), 
			bc.cacheConfig.SnapshotLimit, // 102
			head.Root(), 
			!bc.cacheConfig.SnapshotWait, // true
			true, 
			recover,	// false
		)
	}
	...
	// false
	if bc.cacheConfig.TrieCleanRejournal > 0 { // 60min
...
func (bc *BlockChain) loadLastState() error {
	...
	if head := rawdb.ReadFinalizedBlockHash(bc.db); head != (common.Hash{}) { // false
		if block := bc.GetBlockByHash(head); block != nil {
	...
	currentFinalizedBlock := bc.CurrentFinalizedBlock() // nil
	...
	if currentFinalizedBlock != nil { // false
