...
func New(conf *Config) (*Node, error) {
	...
	conf = &confCopy
	if conf.DataDir != "" {
		// "go-ethereum1.10/XXX_DATA/chain_data"
	...
	// 只是获取一个chain_data/geth 目录锁
	if err := node.openDataDir(); err != nil {
		return nil, err
	}
	// chain_data/keystore, false
...
func (n *Node) obtainJWTSecret(cliParam string) ([]byte, error) {
	...
	if len(fileName) == 0 {
		...
		fileName = n.ResolvePath(datadirJWTKey) // "jwtsecret" "/Users/luke/Desktop/work/go-ethereum1.10/XXX_DATA/chain_data/geth/jwtsecret"
	}
...
func (n *Node) startRPC() error {
	...
	if len(open) != len(all) {
		jwtSecret, err := n.obtainJWTSecret(n.config.JWTSecret) // config.JWTSecret: ""
		if err != nil {
			return err
		}
		if err := initAuth(all, n.config.AuthPort, jwtSecret); err != nil { // 8551
			return err
		}
	}
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, ancient string, namespace string, readonly bool) (ethdb.Database, error) {
	n.lock.Lock() //  name:"chaindata", cache:0, handles:0, ancient: "", namespace: "", readonly:false,
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
	}
