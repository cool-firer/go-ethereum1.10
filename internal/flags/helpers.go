...
func FlagString(f cli.Flag) string {
	...
	namesText := pad(cli.FlagNamePrefixer(df.Names(), placeholder), 30) // "--datadir value               "

	defaultValueString := ""
	if s := df.GetDefaultText(); s != "" {
		defaultValueString = " (default: " + s + ")" // " (default: /Users/luke/Library/Ethereum)"
	}

	usage := strings.TrimSpace(df.GetUsage()) // "Data directory for the databases and keystore"
