...
*/
	...
	res := trie.Hash() // 返回 trie树的root hash -> Hash [32]byte别名
	exp := emptyRoot
	if res != exp {
...
*/
	...
	// 往db.dirties里填充 { node hash值 -> cachedNode {} }
	triedb.Update(NewWithNodeSet(nodes))
	if !memonly {
		// db.dirtes 写入底层kv hash -> node.rlp()
		// 全部写完后, 清空db.dirtes, 仿佛db没来过 
