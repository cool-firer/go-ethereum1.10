...
func NewDatabaseWithConfig(diskdb ethdb.KeyValueStore, config *Config) *Database {
	...
	// cache: 409 journal: "XXX_DATA/chain_data/geth/triecache"
	if config != nil && config.Cache > 0 {
		if config.Journal == "" {
			cleans = fastcache.New(config.Cache * 1024 * 1024)
		} else {
			// "XXX_DATA/chain_data/geth/triecache", 409 * 1024 * 1024
...
// hash: 节点hash, size: 预估size, node: rawShortNode
func (db *Database) insert(hash common.Hash, size int, node node) {
	...
	if db.oldest == (common.Hash{}) {
		db.oldest, db.newest = hash, hash // 首次
	} else {
		// 前一个link到当前这个
...
// 取一个trie树node, 必须是对应的hash, 先从memory, 再从disk
...
// 遍历node的所有children, 写到磁盘, 强制去掉所有关联. 非线程安全
func (db *Database) Commit(node common.Hash, report bool, callback func(common.Hash)) error {
	...
	// 写dirties到底层db
	// 用uncacher去dirties引用
	// hash1 -> (key1, value1)	hash2 -> (addr1, StateAccount)
	...
	// { key: hash, valueL node.rlp() }, {} } 写进db底层 注意顺序, 先写底层的node
	
	// 剩下的写入db, 因为上句commit有个阈值判断
	if err := batch.Write(); err != nil {
...
func (db *Database) commit(hash common.Hash, batch ethdb.Batch, uncacher *cleaner, callback func(common.Hash)) error {
	...
	node, ok := db.dirties[hash] // root node
	if !ok {
		return nil
	}
	var err error
	...
	// 把 { key: hash, value: node.rlp() } 放进 batch.writes里
	...
	if batch.ValueSize() >= ethdb.IdealBatchSize {
		...
		// 以 key: hash -> value: node.rlp() 写入底层数据库
		// memorydb就是一个map
		if err := batch.Write(); err != nil {
			return err
		}
		...
		// 重放? 再 uncaher.put(key, value)
		// 备份一份到uncaher?

		// Replay会遍历写入的数据, 调用uncaher.Put(k, v)
		// 目的是去db.dirties引用, 删除已经持久化的
		batch.Replay(uncacher)
		batch.Reset()
		db.lock.Unlock()
	}
...
func (c *cleaner) Put(key []byte, rlp []byte) error {
	...
	// 去引用
	// 为什么不直接用这一句delete就行了? 要手动去链
	// 因为直接delete虽然可以释放内存, 其他没有delete的key的prev, next可能指向这个, 引起不安全的操作
...
func (db *Database) Update(nodes *MergedNodeSet) error {
	...
	// order: [ addr1, addr2, ... , Hash{} ]
	//				  前面是Stateobject.trie,  后面是state.trie
	...
	if set, present := nodes.sets[common.Hash{}]; present {
		for _, n := range set.leaves {
			// 解码出来
			// statedb.trie 里会有leaves
			var account types.StateAccount
			if err := rlp.DecodeBytes(n.blob, &account); err != nil {
				return err
			}
			if account.Root != emptyRoot {
				// account.Root: StateObject.trie树的hash 存的 key1 -> value1
				// n.parent: satedb.trie树的hash 存的 addr1 -> StateAccount
				// 绿色, 黄色
				// dirties: { 绿色 -> {}, 黄色 -> {children: {绿色: 1}} }
