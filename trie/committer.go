...
func (c *committer) commit(path []byte, n node) (node, error) {
	...
	case *fullNode:
		hashedKids, err := c.commitChildren(path, cn) // path: "12"
		if err != nil {
			return nil, err
		}
		collapsed := cn.copy()
...
func (c *committer) commitChildren(path []byte, n *fullNode) ([17]node, error) {
	...
	for i := 0; i < 16; i++ {
		...
		hashed, err := c.commit(append(path, byte(i)), child) // path: 120, 123
		if err != nil {
			return children, err
		}
		children[i] = hashed
...
func (c *committer) store(path []byte, n node) node {
	...
	var (
		size  = estimateSize(n)
		nhash = common.BytesToHash(hash) // node hash
		mnode = &memoryNode{
			hash: nhash,
			node: simplifyNode(n),
			size: uint16(size),
