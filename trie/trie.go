...
// 上面的意思说了个大概意思: 用现存的root创建一棵trie树, 这个root要在db里面找
// 每棵trie树都有一个owner
// 如果root为zero hash 或 空字符串的sha3(也就是Keccak), 那就初始化一棵空树, 不需要db.
func New(owner common.Hash, root common.Hash, db *Database) (*Trie, error) {
	...
	// emptyRoot是一个Hash 也就是一个[32]byte别名
	// root不是zero hash, 也不是 空串sha3, 就要从db里拿了
	if root != (common.Hash{}) && root != emptyRoot {
		// 从db里找对应hash的node
		// 但注意此时的不同: Node { key: 原始key, val: 下一层的node的hash(hashNode) }
		rootnode, err := trie.resolveHash(root[:], nil)
		if err != nil { // 构建不了, 因为db里没有对应hash的Node
			return nil, err
		}
		trie.root = rootnode
	}
...
func (t *Trie) tryGet(origNode node, key []byte, pos int) (value []byte, newnode node, didResolve bool, err error) {
	...
	case valueNode:
		return n, n, false, nil // 当前已经是个valueNode, 不resolve, 不err
	case *shortNode:
	...
	case *fullNode:
		...
		return value, n, didResolve, err
	case hashNode: // reslove原来是这个作用! 边查找边构建trie树
...
// 如果value长度是0, 就代表delete操作
// value保存在树里后, 千万不能修改, 因为树里与上层指向的是同一个value
...
func (t *Trie) Commit(collectLeaf bool) (common.Hash, *NodeSet, error) {
	...
	// Derive(导出) the hash for all dirty nodes first. We hold the assumption
	// in the following procedure that all nodes are hashed.
	// 从底向上, 算出各个node的hash: node.flag.hash = hash
	// 但node.flag.dirty 依然是 = true
	rootHash := t.Hash()
	...
	if hashedNode, dirty := t.root.cache(); !dirty { // 当前不脏, 直接将root设为hashNode
	...
	// newRoot: hashNode
	// 所有的dirty node 
	// &NodeSet{ owner: addr1, Path: [ path1, ], Nodes: { path1 -> MemoryNode }}
...
func (t *Trie) hashRoot() (node, node, error) {
	if t.root == nil {
		// emptyRoot -> Hash [32]byte别名
		// emtyRoot.Bytes() -> []byte切片
		// 转成hashNode( []byte切片别名 ) 实现了node接口方法
	...
	// hashed:roothash值 cached: 有nodeflag.hash
