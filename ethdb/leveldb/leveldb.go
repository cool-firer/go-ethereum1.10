...
func New(file string, cache int, handles int, namespace string, readonly bool) (*Database, error) {
	return NewCustom(file, namespace, func(options *opt.Options) {
		...
		if handles < minHandles {
			handles = minHandles // 16
		}
		// Set default options
		options.OpenFilesCacheCapacity = handles
		options.BlockCacheCapacity = cache / 2 * opt.MiB // 8 MiB = 1024 * 1024 * 8
		options.WriteBuffer = cache / 4 * opt.MiB        // 4MiB      // Two of these are used internally
		if readonly {
...
func (db *Database) Get(key []byte) ([]byte, error) {
	dat, err := db.db.Get(key, nil) // 不存在就返回nil
	if err != nil {
		return nil, err
	}
	return dat, nil
