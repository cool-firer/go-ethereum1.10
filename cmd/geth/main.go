...
func main() {
	// 创世区块: ../build/bin/geth --datadir "./chain_data" init genesis.json
	// os.Args: [../build/bin/geth, --datadir, ./chain_data, init, genesis.json
...
`)
	...
	case !ctx.IsSet(utils.NetworkIdFlag.Name): // true
		log.Info("Starting Geth on Ethereum mainnet...")
	}
	// If we're a full node on mainnet without --cache specified, bump default cache allowance
	if ctx.String(utils.SyncModeFlag.Name) != "light" && !ctx.IsSet(utils.CacheFlag.Name) && !ctx.IsSet(utils.NetworkIdFlag.Name) { // true
