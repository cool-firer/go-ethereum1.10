...
func setListenAddress(ctx *cli.Context, cfg *p2p.Config) {
	if ctx.IsSet(ListenPortFlag.Name) { // false
		cfg.ListenAddr = fmt.Sprintf(":%d", ctx.Int(ListenPortFlag.Name))
	}
	if ctx.IsSet(DiscoveryPortFlag.Name) { // false
...
func MakeDatabaseHandles(max int) int {
	limit, err := fdlimit.Maximum() // 10240
	if err != nil {
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	lightClient := ctx.String(SyncModeFlag.Name) == "light" // false
	lightServer := (ctx.Int(LightServeFlag.Name) != 0) // false
	...
	ethPeers := cfg.MaxPeers - lightPeers // 50 - 0
	if lightClient {
		ethPeers = 0
	}
	log.Info("Maximum peer count", "ETH", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)

	if ctx.IsSet(MaxPendingPeersFlag.Name) { // false
		cfg.MaxPendingPeers = ctx.Int(MaxPendingPeersFlag.Name)
	}
	if ctx.IsSet(NoDiscoverFlag.Name) || lightClient { // true
...
func CheckExclusive(ctx *cli.Context, args ...interface{}) {
	...
	for i := 0; i < len(args); i++ {
		...
		name := flag.Names()[0] // "ipcdisable" / "ipcpath"
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	cache := ctx.Int(CacheFlag.Name) // 2730
	...
	if ctx.IsSet(CacheFlag.Name) || ctx.IsSet(CacheDatabaseFlag.Name) {
		cfg.DatabaseCache = ctx.Int(CacheFlag.Name) * ctx.Int(CacheDatabaseFlag.Name) / 100 // 1365
	}
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Ethereum) {
	...
	// 注册一个namespace:engine rpc
	if err := ethcatalyst.Register(stack, backend); err != nil {
		Fatalf("Failed to register the Engine API service: %v", err)
	}

	// 注册一个namespace:debug rpc
	stack.RegisterAPIs(tracers.APIs(backend.APIBackend))
