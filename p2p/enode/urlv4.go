...
func NewV4(pubkey *ecdsa.PublicKey, ip net.IP, tcp, udp int) *Node {
	var r enr.Record
	if len(ip) > 0 {
		r.Set(enr.IP(ip))// 填充 r.pairs: append( &pair{"ip", RLP(ip)}  )
	}
	if udp != 0 {
		r.Set(enr.UDP(udp)) // r.pairs: append( &pair{"dup", RLP(udp)}  )
	}
	if tcp != 0 {
		r.Set(enr.TCP(tcp)) // append &pair{"tcp", RLP(tcp)} }
	}
	signV4Compat(&r, pubkey) // append &piar{“secp256k1”， RLP(pubkey)}
